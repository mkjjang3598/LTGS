...
def object_pcds_from_mast3r(dataset: ModelParams, change_cameras: list, timestep:int, temporal_indices: list):
    ...
    capture_dir = os.path.join(output_dir, "change", "renders")
    ...
    for file in filenames:
        obj_mask = np.load(os.path.join(instance_dir, "renders", file.split('.')[0]+'.npy'))
        # Also load mask for removed regions (negative values)
        rm_obj_mask = np.load(os.path.join(instance_dir, "capture", file.split('.')[0]+'.npy'))
    ...
    mast3r_recon_path = os.path.join(output_dir, "mast3r", f"change_recon_0.npz")
    ...
    pcd_color = np.concatenate(pcd_color, 0)
