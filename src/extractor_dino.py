...
# from utils.visualization_utils import compute_pca_image
...
class ViTExtractor:
    ...
    def preprocess_inputfile(self, inputfiles: List):
        ...
        new_width = divisible_by_num(patch_size, width)
        new_height = divisible_by_num(patch_size, height)
        pil_imgs = [pil_img.resize((new_width, new_height), resample=Image.LANCZOS) for pil_img in pil_imgs]
    
        prep = transforms.Compose([
            transforms.ToTensor(),
            transforms.Normalize(mean=self.mean, std=self.std)
        ])
        prep_img = [prep(pil_img) for pil_img in pil_imgs]
        prep_img = torch.stack(prep_img, dim=0)
        return prep_img, new_height, new_width
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_type', default='dino_vitb16', type=str,
    ...
    with torch.no_grad():
        ...
        descriptors = descriptors.reshape(1, num_patches[0],num_patches[1], descriptors.shape[-1]).squeeze(0).detach().cpu().numpy()
        # dino_pca = compute_pca_image(descriptors)
        # plt.imshow(dino_pca)
        # plt.show()