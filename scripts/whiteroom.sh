...
CUDA_VISIBLE_DEVICES=${GPU} python change_detection.py -m gaussian-splatting/${OUTPUT} --min_size 1500 --max_size 300000 --kernel_size 5 --cosine_thr 0.92 # --manual_selection # Change detection

# Instance matching
CUDA_VISIBLE_DEVICES=${GPU} python instance_matching.py -m gaussian-splatting/${OUTPUT} --filter_consistent --num_match_thres 50 --multi_similarity_thres 0.9 --optim_level refine
CUDA_VISIBLE_DEVICES=${GPU} python pcd_initialization.py -m gaussian-splatting/${OUTPUT} --slackness -0.8

# Updating
# CUDA_VISIBLE_DEVICES=${GPU} python long_term_update.py -m gaussian-splatting/${OUTPUT} --skip_localization --prune_dist_thres 0.03 --optimizer_type default  --invalid_initialization 1,2,3,4 --initial_time_loss_weight 0.1 --refine_iterations 1500
# v2
